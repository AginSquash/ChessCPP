...
//      ИЛИ значение типа figure_type. Например вместо "bB" испольщовать 0 (ИЛИ
//      b_Bishop), "bK" -> 1 и т.д.
...
void inputInSave(int IndexFigure,
    sf::Vector2f pos,
    int FieldIndex,
    ofstream* ChessMoves)
{
...
int GetFigureByPosition(
    chess_figure* p_figures,
    sf::Vector2f pos) //Функция находит фигуру соотв. координатам
{
...
bool figurekillforpawns(chess_figure* p_figures,
    int field_index,
    int figure_to_move_index,
    sf::Vector2f pos)
{
...
void figureKill(chess_figure* p_figures,
    short field_index,
    short figure_to_move_index,
    sf::Vector2f pos)
{
...
int main()
{
    ...
    ofstream ChessMoves(
        resource_path + "Save.txt",
        ios_base::trunc); // Связываем класс с файлом и чистим его. Пока это
        // тестовый файл, потом поменяем.
    ofstream filesave(resource_path + "fuck_file.txt",
        ios_base::trunc); //Описание fuck_file есть в документации
    ...
    //Функция из "configWorker.cpp". Читает конфиг и возвращает нужную папку с
    //теустурами шахмат
    ...
    bool isClicked = false; // Перемнная, которая хранит состояние мышки. Если false - то это
        // "первый" клик. Если true - то это "второй" клик, а значит нужно
        // передвинуть фигуру выбранную на первом клике в(???) координаты
        // второго клика
        ...
        while (window.pollEvent(event) && (p_figures[5].isAlive == 1 && p_figures[21].isAlive == 1)) {
            ...
            case sf::Event::MouseButtonReleased: //Если нажимем кнопку на мыш(и)
                if (event.mouseButton.button == sf::Mouse::Left) {
                    sf::Vector2i mouse_world = sf::Mouse::getPosition(
                        window); //Получаем координаты мышки на экране
                    ...
                    if (!isClicked) { //Если это "первый" клик
                        ...
                        isPosibleMoves(p_figures[figure_to_move_index].type, pos,
                            p_figures, scale, freePositions);

                    } else {
                        if (figure_to_move_index != -1) { // Если мы до этого выбрали пустую область - не стоит
                            // что-либо делать

                            short field_index = GetFigureByPosition(
                                p_figures, pos); // Получаем фигуру по координатам нажатия
                            if (field_index == -1) // Если GetFigureByPosition возвращает
                            // -1, значит мы нажимаем на
                            //      пустую клетку
                            {
                            ...
                            if (field_index != -1) {
                                if (((figure_to_move_index > 7 && figure_to_move_index < 16) || (figure_to_move_index > 23 && figure_to_move_index < 31)) && (figurekillforpawns(p_figures, field_index, figure_to_move_index, pos))) {
                                    p_figures[figure_to_move_index].position = pos;
                                    figureKill(p_figures, field_index, figure_to_move_index,
                                        pos);
                                }
                            } else {
                                if ((field_index < 16) && (figure_to_move_index >= 16))
                                /* Значит field_index белая, а figure_to_move_index - черная
                     поэтому мы убираем field_index, а на ее место */
                                ...
                                /* Та же функция что и выше, но при условии что field_index
                     черная, а figure_to_move_index - белая */
                                {
                                    figureKill(p_figures, field_index, figure_to_move_index,
                                        pos);
                                    inputInSave(figure_to_move_index, pos, field_index,
                                ...
                                /* Этот if фиксит вывод надписи, когда несколько раз жмякаешь
                     на одну фигуру */
                                {
                                    popup_text.setString(
                                        "This position is taken by an allied figure.");
...
#ifdef WINDOWS
    for (int i = 0; i < 32; i++) {
        filesave << "figure_type[" << i << "] = " << p_figures[i].position.x << "\t"
                 << p_figures[i].position.y << "\tIs live - "
                 << p_figures[i].isAlive << "\n";
    }
#endif
